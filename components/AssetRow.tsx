...
export function AssetRow({
  ...
  return (
    <div className={`asset-row ${compact ? "asset-row--compact" : ""}`} role="row">
      <Link className="asset-cell asset-cell--name asset-cell--link" href={`/asset/${encodeURIComponent(canonicalAssetSymbol(asset))}`} role="cell">
      ...
      <div className="asset-cell asset-actions" role="cell">
        ...
        <button aria-label={isWatched ? (en ? `Remove ${asset.symbol} from watchlist` : `移除 ${asset.symbol} 自选`) : (en ? `Add ${asset.symbol} to watchlist` : `将 ${asset.symbol} 加入自选`)} className="row-action row-action--icon" data-active={isWatched} onClick={() => onToggleWatchlist(canonicalAssetId(asset), asset.id)} type="button"><Star size={17} weight={isWatched ? "fill" : "regular"} /></button>
      </div>
    </div>
  );
}
