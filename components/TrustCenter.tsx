...
export function TrustCenter() {
  ...
  return <div className="trust-center">
    ...
    <section className="trust-method-grid"><article><Eye size={23} /><h2>{en ? "Editorial discipline" : "新闻编辑纪律"}</h2><ul><li>{en ? "Daily Pulse promotes only market-relevant, event-level stories." : "每日热点只提升与币圈、币股直接相关的事件级新闻。"}</li><li>{en ? "Confirmed facts, inference and unverified impact stay separated." : "已确认事实、推断与未确认影响分开呈现。"}</li><li>{en ? "Generic roundups, clipped headlines and failed translations remain outside the share digest." : "泛化汇总、截断标题和未通过校验的翻译不会进入分享摘要。"}</li><li>{en ? "The full 7×24 page retains the wider raw flow with source links." : "完整 7×24 页面保留更广的信息流与原始链接。"}</li></ul></article><article><LockKey size={23} /><h2>{en ? "Privacy-friendly product analytics" : "隐私友好的产品分析"}</h2><ul><li>{en ? "No analytics cookie, advertising ID, wallet address or account identifier." : "不设置分析 Cookie，不采集广告 ID、钱包地址或账户标识。"}</li><li>{en ? "Raw IP addresses and full user-agent strings are never stored; rotating salted hashes and coarse device categories are used for aggregate visitor counts." : "不保存原始 IP 与完整 User-Agent；只用定期轮换的加盐哈希和粗粒度设备分类形成汇总访客统计。"}</li><li>{en ? "Only allow-listed event names, route paths without query parameters, source domains and coarse feature properties are logged." : "只记录白名单事件名、不含查询参数的页面路径、来源域名和粗粒度功能属性。"}</li><li>{en ? "Browser Do Not Track is respected, and aggregate analytics expires after the stated retention window." : "尊重浏览器 Do Not Track 设置，汇总分析会在公开的保留期后清理。"}</li><li>{en ? "Watchlists, alerts and pause records stay local unless you export them or explicitly opt into wallet sync." : "自选、提醒与冷静记录默认只在本机，除非你主动导出或明确启用钱包同步。"}</li></ul></article></section>
