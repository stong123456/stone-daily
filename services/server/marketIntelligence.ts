...
export async function collectMarketIntelligence(symbol: string): Promise<MarketIntelligence> {
  const normalized = symbol.trim().toUpperCase();
  const results = await Promise.allSettled([fetchBinance(normalized), fetchBybit(normalized), fetchOkx(normalized)]);
